...
noncomputable def kron {q : ℕ} (A : Matrix m n) (B : Matrix p q)
    : Matrix (m * p) (n * q) :=
...
theorem proj_add (s t : Vector n)
    : proj (s + t) = proj s + mul s (adjoint t) + mul t (adjoint s) + proj t := by
  ext i j
...
@[simp]
theorem kron_zero_left {q : ℕ} (B : Matrix p q) : kron (0 : Matrix m n) B = 0 := by
  ext i j
...
@[simp]
theorem kron_zero_right {q : ℕ} (A : Matrix m n) : kron A (0 : Matrix p q) = 0 := by
  ext i j
  rcases finProdFinEquiv.symm i with ⟨i₁, i₂⟩
  rcases finProdFinEquiv.symm j with ⟨j₁, j₂⟩
  simp [kron]
...
theorem kron_mul_assoc {r s t u : ℕ} (A : Matrix m n) (B : Matrix p q)
    ...
    : mul (mul (kron A B) (kron C D)) (kron E F)
      = kron (mul (mul A C) E) (mul (mul B D) F) := by
  simp [kron_mul]
...
theorem proj_add_kron (s t : Vector m) (u v : Vector n)
    : proj (kron s u + kron t v)
      = kron (proj s) (proj u)
        + kron (mul s (adjoint t)) (mul u (adjoint v))
        + kron (mul t (adjoint s)) (mul v (adjoint u))
        + kron (proj t) (proj v) := by
...
theorem isUnitary_mul {A B : Square n} (hA : isUnitary A) (hB : isUnitary B)
    : isUnitary (mul A B) := by
  rw [isUnitary_iff_adjoint_mul_self]
  rw [adjoint_mul]
...
theorem isUnitary_kron {A : Square m} {B : Square n} (hA : isUnitary A) (hB : isUnitary B)
    : isUnitary (kron A B) := by
  rw [isUnitary_iff_adjoint_mul_self]
...
theorem isNormalized_kron {m n : ℕ} {s : Vector m} {t : Vector n}
    (hs : IsNormalized s) (ht : IsNormalized t)
    : IsNormalized (s ⊗ t) := by
